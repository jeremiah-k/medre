...
class TestContradictoryDuplicateSourceHistoryRejected:
    """Duplicate queued receipts across sources cannot outrank the envelope."""

    async def test_duplicate_source_history_rejects_callback(
        self,
        temp_storage: StorageBackend,
    ) -> None:
        """Live and replay queued receipts on one row/attempt contradict the
        single-sourced attempt; the envelope fence rejects the callback."""
        lifecycle = _make_lifecycle()
        ...
        sent = [r for r in all_receipts if r.status == "sent"]
        assert len(sent) == 0
        row = await temp_storage.get_outbox_item("obox-dup-src")
        assert row is not None
        assert row.status == "queued"

    async def test_matching_envelope_finalizes_with_envelope_lineage(
        self,
        temp_storage: StorageBackend,
    ) -> None:
        """A single queued receipt agreeing with the envelope finalizes and the
        supplemental sent receipt carries the envelope's replay lineage."""
        lifecycle = _make_lifecycle()
        now = datetime.now(timezone.utc)
        ...
        await create_outbox_item_with_parent(
            temp_storage, outbox_item, allocate_new_generation=True
        )
        ...
        assert len(sent) == 1
        assert sent[0].source == "replay"
        assert sent[0].replay_run_id == "run-7"
        assert sent[0].parent_receipt_id == "rcpt-run"
