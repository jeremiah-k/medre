...
1. stop() waits for an in-flight ``_report_deferred_failure`` callback
2. stop() waits for an in-flight ``_report_deferred_completion`` callback
...
class TestStopWaitsForTerminalCallback:
    """stop() waits for in-flight _report_deferred_failure to complete."""
    ...
    async def test_terminal_callback_completes_before_stop_returns(
        ...
        adapter.ctx = _make_ctx(report_delivery_feedback=slow_terminal)
...
class TestStopWaitsForNativeRefCallback:
    """stop() waits for in-flight _report_deferred_completion to complete."""
    ...
    async def test_native_ref_callback_completes_before_stop_returns(
        ...
        adapter.ctx = _make_ctx(report_delivery_feedback=slow_native_ref)
        adapter._started = True
...
class TestCallbackExceptionLogged:
    ...
    async def test_callback_exception_logged_not_unobserved(self) -> None:
        ...
        caught by _report_deferred_failure's own try/except (logged) and
        ...
        adapter.ctx = _make_ctx(report_delivery_feedback=failing_terminal)
        adapter._started = True
...
class TestNoUntrackedTasksAfterStop:
    ...
    async def test_no_untracked_tasks_after_stop(self) -> None:
        ...
        adapter.ctx = _make_ctx(report_delivery_feedback=noop_terminal)
        adapter._started = True
...
class TestDelayedOutboundRefExceptionLogged:
    """When _report_deferred_completion raises a non-CancelledError
    ...
    async def test_native_ref_exception_logged_in_drain_loop(self) -> None:
        ...
        adapter.ctx = _make_ctx(report_delivery_feedback=failing_native_ref)
        adapter._started = True
...
# ===================================================================
# 7. _report_deferred_failure callback raises (lines 1029, 1033)
# ===================================================================


class TestReportQueueTerminalCallbackRaises:
    """_report_deferred_failure contains exceptions from the unified feedback sink."""

    async def test_callback_exception_caught_and_logged(self) -> None:
        """A failing feedback sink is contained and does not propagate."""
        ...
        await adapter._report_deferred_failure(result)

    async def test_callback_exception_with_no_ctx(self) -> None:
        """_report_deferred_failure with ctx=None does not crash when the
...
class TestReportCancelledAndDrainPaths:
    ...
    async def test_cancelled_item_with_callback_reports_cancelled(self) -> None:
        ...
        a 'cancelled' DeferredHandoffFailed is sent to the callback."""
        ...
        adapter.ctx = _make_ctx(report_delivery_feedback=capture_terminal)
    ...
    async def test_abandoned_callback_exception_logged(self) -> None:
        ...
        adapter.ctx = _make_ctx(report_delivery_feedback=failing_on_second)
    ...
    async def test_no_callback_drain_all_silent(self) -> None:
        ...
        config = make_meshtastic_config(connection_type="fake")
        adapter = MeshtasticAdapter(config)
        # No report_delivery_feedback sink → callback is None.
        adapter.ctx = _make_ctx()
...
class TestNativeRefExceptionAfterCancel:
    ...
    async def test_native_ref_exception_observed_after_cancel(self) -> None:
        ...
        adapter.ctx = _make_ctx(report_delivery_feedback=slow_failing_native_ref)
        adapter._started = True
    ...
    async def test_terminal_exception_observed_after_cancel(self) -> None:
        ...
        _report_deferred_failure, so the shielded task completes normally
        ...
        # _report_deferred_failure, so _background_tasks should be empty
