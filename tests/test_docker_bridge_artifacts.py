...
class TestGetArtifactPlan:
    ...
    def test_matrix_to_meshtastic_requires_synapse_log(self) -> None:
        plan = get_artifact_plan("matrix_to_meshtastic")
        assert "synapse.log" in plan["required"]
        assert "meshtasticd.log" not in plan["required"]
...
class TestCollectArtifactManifestScenarioAware:
    ...
    def test_matrix_scenario_missing_meshtasticd_not_reported(self, tmp_path: Path) -> None:
        ...
        # meshtasticd.log should NOT be in required at all for matrix scenario.
...
class TestScenarioAwareMissingArtifacts:
    ...
    def test_matrix_scenario_does_not_report_meshtasticd_missing(self, tmp_path: Path) -> None:
        mock_runner = self._make_mock_runner(stdout="1 passed in 1s\n")
        summary = collect_docker_bridge_artifacts(
            scenario="matrix_to_meshtastic",
            base_dir=tmp_path / "runs",
        ...
        missing_req = summary["missing_artifacts"]["required"]
        assert "meshtasticd.log" not in missing_req
        assert "synapse.log" in missing_req

    def test_meshtastic_scenario_does_not_report_synapse_missing(self, tmp_path: Path) -> None:
        mock_runner = self._make_mock_runner(stdout="1 passed in 1s\n")
        summary = collect_docker_bridge_artifacts(
            scenario="meshtastic_to_matrix",
            base_dir=tmp_path / "runs",
    ...
    def test_matrix_scenario_artifact_plan_has_correct_required(self, tmp_path: Path) -> None:
        mock_runner = self._make_mock_runner(stdout="1 passed in 1s\n")
        summary = collect_docker_bridge_artifacts(
            scenario="matrix_to_meshtastic",
            base_dir=tmp_path / "runs",
    ...
    def test_meshtastic_scenario_artifact_plan_has_correct_required(self, tmp_path: Path) -> None:
        mock_runner = self._make_mock_runner(stdout="1 passed in 1s\n")
        summary = collect_docker_bridge_artifacts(
            scenario="meshtastic_to_matrix",
            base_dir=tmp_path / "runs",
