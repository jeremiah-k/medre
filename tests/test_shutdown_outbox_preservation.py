...
class TestPendingPreservedAcrossShutdown:
    ...
    async def test_pending_status_preserved(self) -> None:
...
class TestRetryWaitPreservedAcrossShutdown:
    ...
    async def test_retry_wait_status_preserved(self) -> None:
        f = tempfile.NamedTemporaryFile(suffix=".db", delete=False)
        db_path = f.name
        f.close()
        try:
...
class TestInProgressPreservedAcrossShutdown:
    ...
    async def test_in_progress_status_preserved_with_lease(self) -> None:
        f = tempfile.NamedTemporaryFile(suffix=".db", delete=False)
        db_path = f.name
        f.close()
        try:
...
class TestTerminalStatusesPreservedAcrossShutdown:
    ...
    async def test_terminal_status_preserved(self, terminal_status: str) -> None:
        f = tempfile.NamedTemporaryFile(suffix=".db", delete=False)
        db_path = f.name
        f.close()
        try:
...
class TestMixedStatusesPreservedAcrossShutdown:
    ...
    async def test_all_statuses_preserved(self) -> None:
...
class TestNoSpuriousCancelledOrAbandoned:
    ...
    async def test_counts_identical_before_and_after_shutdown(self) -> None:
...
class TestShutdownEvidenceFromLiveStorageCounts:
    ...
    async def test_evidence_graceful_stop_when_all_terminal(self) -> None:
...
class TestClassifyOutboxPolicyFromStorageItems:
    ...
    async def test_nonterminal_classified_resumable(self, status: str) -> None:
    ...
    async def test_terminal_classified_no_resume(self, status: str) -> None:
    ...
    async def test_retry_wait_claimable_when_due_after_reopen(self) -> None:
