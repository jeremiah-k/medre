...
class TestRenderingFailure:
    ...
    async def test_provenance_contradiction_follows_renderer_failure_path(self) -> None:
        """A renderer result contradicting the attempt envelope must fail as
        RENDERER_FAILURE evidence, not escape as a raw validation error."""
        pipeline = _FakeRenderingPipeline(
            result=RenderingResult(
                event_id="evt-renderer-bug",
                target_adapter="test_adapter",
                target_channel=None,
                payload={"text": "hello"},
            )
        ...
        assert err.receipt.outbox_id == "obox-prov-fail"
        assert err.receipt.attempt_number == 1
