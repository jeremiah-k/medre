...
class TestBoundedVsRudeWork:
    async def test_returns_at_deadline_when_work_ignores_cancellation(self) -> None:
    ...
    async def test_primary_exception_propagates_with_label(self) -> None:
        async def boom() -> None:
...
class TestLaunchBoundedCleanup:
    async def test_start_failure_stops_app_and_reraises_primary(self) -> None:
    ...
    async def test_hanging_start_hits_deadline_then_cleans_up(self) -> None:
    ...
    async def test_successful_start_returns_app_without_stop(self) -> None:
        app = _FakeApp()
