"""Evidence bundle LXMF adapter diagnostics integration test.

Verifies that the evidence bundle correctly handles a configuration that
includes an LXMF adapter (in fake mode). This is NOT a live test — it
uses fake adapters to exercise the evidence collection pipeline without
network access.

LXMF-specific checks:
- diagnostics_snapshot section includes adapter metadata
- config_summary section includes LXMF adapter
- route validation works with LXMF routes
- storage section can query LXMF-shaped events
- no secrets leak into the evidence bundle output
"""
...
class TestEvidenceBundleWithLxmfAdapter:
    """Evidence bundle correctly handles LXMF adapter in config."""

    async def test_evidence_bundle_collects_with_lxmf_config(self, tmp_path) -> None:
        """collect_evidence_bundle succeeds with an LXMF adapter in config."""
    ...
    async def test_evidence_bundle_config_summary_includes_lxmf(self, tmp_path) -> None:
        ...
        # Should include the lxmf adapter
        adapter_ids = [a.get("adapter_id", "") for a in adapters]
        assert (
            "test_lxmf" in adapter_ids
        ), f"LXMF adapter not found in enabled adapters: {adapter_ids}"

    async def test_evidence_bundle_diagnostics_includes_lxmf_adapter(
        self, tmp_path
    ) -> None:
        """Diagnostics snapshot includes LXMF adapter metadata."""
        config_path = _lxmf_fake_config_path(tmp_path)
        bundle = await asyncio.wait_for(
            collect_evidence_bundle(config_path), timeout=_EVIDENCE_TIMEOUT
        )
        ...
        adapters = data.get("adapters", {})
        assert (
            "test_lxmf" in adapters
        ), f"LXMF adapter not in diagnostics adapters: {list(adapters.keys())}"

        lxmf_adapter = adapters["test_lxmf"]
        # The fake adapter must report standard fields.
        assert "adapter_id" in lxmf_adapter
        assert lxmf_adapter["adapter_id"] == "test_lxmf"
        assert "platform" in lxmf_adapter
        assert "health" in lxmf_adapter
        assert "capabilities" in lxmf_adapter

    async def test_evidence_bundle_route_validation_includes_lxmf_route(
        self, tmp_path
    ) -> None:
        """Route validation section shows the Matrix→LXMF bridge route."""
        config_path = _lxmf_fake_config_path(tmp_path)
        bundle = await asyncio.wait_for(
            collect_evidence_bundle(config_path), timeout=_EVIDENCE_TIMEOUT
        )
        ...
        assert any(
            ...
            and "test_lxmf" in r.get("dest_adapters", [])
            for r in routes
        ), f"Expected Matrix->LXMF route in config_summary routes: {routes}"
    ...
    async def test_evidence_bundle_storage_path_mode_with_lxmf_event(
        self, tmp_path
    ) -> None:
        """Storage-path direct mode can inspect LXMF-shaped events."""
        ...
        # Storage section should have the LXMF event
        ...
        assert data.get("event") is not None, "LXMF event not found in storage"

        # Verify the event has LXMF-specific fields
        stored_event = data["event"]
        assert stored_event["source_adapter"] == "lxmf-alpha"
        assert stored_event["source_channel_id"] is None

        # Native refs should include the LXMF native ref
        native_refs = data.get("native_refs_for_event", [])
        assert len(native_refs) >= 1, "Expected at least one native ref"

        # At least one native ref must match the stored LXMF ref fields
        match = any(
            nr.get("adapter") == "lxmf-alpha"
            and nr.get("native_channel_id") is None
            and nr.get("native_message_id") == "cd" * 32
            and nr.get("direction") == "inbound"
            for nr in native_refs
        )
        assert match, (
            f"No native ref matched expected LXMF inbound ref. "
            f"native_refs={native_refs}"
        )

        # Storage event should include source_native_ref or native metadata
        # with the LXMF message_id
        assert stored_event.get("source_native_ref") is not None or (
            ...
            .get("message_id")
            == "cd" * 32
        ), "Storage event missing source_native_ref and native metadata message_id"

        # Verify incident summary
        incident = data.get("incident_summary")
        assert incident is not None
        assert incident["source_adapter"] == "lxmf-alpha"
