...
class TestLoopSuppressionGoldenFlow:
    ...
    async def test_loop_suppression_golden_flow(self, tmp_path: Path) -> None:
        ...
        stored_receipts = await storage.list_receipts_for_event(event.event_id)
        ...
        # Cleanup runner + adapter; then close storage for evidence read.
        ...
        assert data["receipt_count"] >= 1, "At least one receipt in storage"
