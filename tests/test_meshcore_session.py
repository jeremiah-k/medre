...
def _make_tcp_session() -> tuple[MeshCoreSession, AsyncMock]:
    """Create a TCP session with connected=True and a mock _meshcore."""
    config = _make_config(connection_type="tcp", host="localhost")
    session = MeshCoreSession(config, "retry-delay-session")
    session._diag.connected = True
...
class TestPerContactRetryDelayCache:
    """Per-contact _contact_retry_delays dict replaces session-global delay."""

    async def test_timeout_for_contact_a_used_for_contact_a_retry(self) -> None:
        """Send to 'alpha' with suggested_timeout=2000, then fail+retry to
        'alpha' — retry sleep should use 2.0 s."""
        from unittest.mock import patch
        ...
        # First call succeeds and caches delay for "alpha".
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            await session.send_text("alpha", "first")

        # Second call: first attempt fails, retry should use cached 2.0s.
        call_count = 0
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            await session.send_text("alpha", "second")
            # Find the retry sleep (not the pacing sleep).
            retry_sleeps = [
                c
    ...
    async def test_timeout_for_contact_a_not_used_for_contact_b(self) -> None:
        """Cached delay for 'alpha' is not used when retrying 'beta'."""
        from unittest.mock import patch

        session, mock_mc = _make_tcp_session()

        # Send to "alpha" with suggested_timeout=2000 to cache it.
        mock_mc.commands.send_msg.return_value = {
            "expected_ack": b"\x01\x02",
            "suggested_timeout": 2000,
        }
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock):
            await session.send_text("alpha", "cache alpha")

        assert session._contact_retry_delays.get("alpha") == 2.0

        # Now send to "beta" — fail once, then succeed.
        call_count = 0
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            await session.send_text("beta", "retry beta")
            retry_sleeps = [
                c
                for c in mock_sleep.call_args_list
                if c.args[0] != session._config.message_delay_seconds
            ...
            assert len(retry_sleeps) >= 1
            # Should use fallback 0.1 * attempt = 0.1, NOT alpha's 2.0.
            assert retry_sleeps[0].args[0] == 0.1

    async def test_timeouts_coexist_for_multiple_contacts(self) -> None:
        """Delays cached independently for multiple contacts."""
        from unittest.mock import patch
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock):
            await session.send_text("alpha", "msg1")
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock):
            await session.send_text("beta", "msg2")

        assert session._contact_retry_delays["alpha"] == 2.0
        assert session._contact_retry_delays["beta"] == 5.0

        # Retry alpha — should use 2.0s.
        call_count = 0
        ...
        async def _fail_once_ok(*args, **kwargs):
            ...
            if call_count == 1:
                raise RuntimeError("transient")
            return {"expected_ack": b"\x11\x12"}

        mock_mc.commands.send_msg.side_effect = _fail_once_ok

        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            await session.send_text("alpha", "retry alpha")
            retry_sleeps = [
                c
                for c in mock_sleep.call_args_list
                if c.args[0] != session._config.message_delay_seconds
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            ...
            assert retry_sleeps[0].args[0] == 5.0

    async def test_cache_clears_on_stop(self) -> None:
        """stop() clears the per-contact retry delay cache."""
        from unittest.mock import patch
        ...
        assert len(session._contact_retry_delays) == 2

        await session.stop()
        assert len(session._contact_retry_delays) == 0

    async def test_channel_send_never_consults_contact_cache(self) -> None:
        """Channel send retry does NOT use cached DM contact delays."""
        from unittest.mock import patch

        session, mock_mc = _make_tcp_session()

        # Cache alpha = 2.0s via DM.
        mock_mc.commands.send_msg.return_value = {
            "expected_ack": b"\x01\x02",
            "suggested_timeout": 2000,
        }
        with patch("asyncio.sleep", new_callable=AsyncMock):
            await session.send_text("alpha", "dm msg")

        assert session._contact_retry_delays.get("alpha") == 2.0

        # Now send a channel message that fails once.
        call_count = 0
    ...
    async def test_diagnostics_contact_timeout_count(self) -> None:
        """Diagnostics includes sdk_contact_timeout_count reflecting cache size."""
        from unittest.mock import patch
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock):
            await session.send_text("alpha", "msg1")
            await session.send_text("beta", "msg2")
            await session.send_text("gamma", "msg3")

        diag = session.diagnostics()
        assert diag["sdk_contact_timeout_count"] == 3
