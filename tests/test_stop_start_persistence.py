...
The key architectural insight: two different PipelineRunner instances can point
to the same SQLite database file, and data written by the first runner is
visible to the second after re-initialization.
...
@pytest.mark.asyncio
async def test_events_survive_stop_start() -> None:
    ...
    try:
        ...
        # Events should still be in storage before close.
...
@pytest.mark.asyncio
async def test_receipts_survive_stop_start() -> None:
...
@pytest.mark.asyncio
async def test_native_ref_dedup_survives_restart() -> None:
    ...
    try:
        ...
        # The event was stored and native ref persisted.
        ...
        # The duplicate event was never stored.
...
# ---------------------------------------------------------------------------
# Test 4: Accounting resets on new runner (process-local)
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_accounting_resets_on_new_runner() -> None:
    ...
    regardless of what the previous runner processed.
    """
    db_path = _temp_db_path()
    try:
        route = _make_bridge_route()
        ...
        # not reset by runner B's creation.
        assert snap_a["inbound_accepted"] == 3, (
