...
class TestCreateAndGet:
    ...
    async def test_create_and_get_round_trip(self, outbox_temp_storage: SQLiteStorage) -> None:
        ...
        retrieved = await outbox_temp_storage.get_outbox_item(item.outbox_id)
        assert retrieved is not None
        assert retrieved.outbox_id == item.outbox_id
        assert retrieved.event_id == "__outbox_default__"
    ...
    async def test_get_returns_none_for_unknown(
        ...
        result = await outbox_temp_storage.get_outbox_item("does-not-exist")
...
class TestListOutboxItems:
    ...
    async def test_list_all(self, outbox_temp_storage: SQLiteStorage) -> None:
    ...
    async def test_list_by_status(self, outbox_temp_storage: SQLiteStorage) -> None:
        ...
        created_sent = await outbox_temp_storage.create_outbox_item(sent_base)
    ...
    async def test_list_limit_and_offset(self, outbox_temp_storage: SQLiteStorage) -> None:
        ...
        page1 = await outbox_temp_storage.list_outbox_items(limit=2, offset=0)
        assert len(page1) == 2
...
class TestCountByStatus:
    ...
    async def test_counts(self, outbox_temp_storage: SQLiteStorage) -> None:
        ...
        await outbox_temp_storage.create_outbox_item(pending1)
        await outbox_temp_storage.create_outbox_item(pending2)
        created_sent = await outbox_temp_storage.create_outbox_item(sent_base)
        await outbox_temp_storage.mark_outbox_sent(
            created_sent.outbox_id, receipt_id="rcpt-sent"
        )

        counts = await outbox_temp_storage.count_outbox_by_status()
