...
# ---------------------------------------------------------------------------
# Bring-up runbook path (used by tests 21–27)
# ---------------------------------------------------------------------------
...
class TestBringupRunbookAuthFirst:
    ...
    ``medre auth matrix login`` *before* running ``medre run``."""

    def test_bringup_mentions_auth_login(self) -> None:
        """bringup runbook must contain ``medre auth matrix login``."""
        if not _BRINGUP.exists():
            pytest.skip("bringup runbook not found")
        text = _read(_BRINGUP)
        assert "medre auth matrix login" in text, (
            "live-matrix-meshtastic-bringup.md must mention "
            "'medre auth matrix login' in the auth-first workflow."
        )

    def test_auth_appears_before_run(self) -> None:
        """The first ``medre auth`` mention must appear before the first
        ``medre run`` mention."""
        if not _BRINGUP.exists():
            pytest.skip("bringup runbook not found")
        text = _read(_BRINGUP)
        auth_pos = text.find("medre auth")
        run_pos = text.find("medre run")
        assert auth_pos >= 0, (
            "bringup runbook must mention 'medre auth matrix login'."
        )
        assert run_pos >= 0, (
            "bringup runbook must mention 'medre run'."
        )
        assert auth_pos < run_pos, (
            "bringup runbook must present 'medre auth' before 'medre run' "
...
class TestSecureCredentialsBearerToken:
    ...
    def test_mentions_auth_command(self) -> None:
        ...
        text = _read(_SECURE_CREDS)
        assert "medre auth matrix login" in text, (
            "secure-credentials.md must mention 'medre auth matrix login' "
...
class TestOperatorAnswerability:
    ...
    def test_how_token(self) -> None:
        ...
        assert "medre auth matrix login" in text, (
            ...
            "token via 'medre auth matrix login'."
        )
