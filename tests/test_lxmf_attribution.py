"""Tests for strict versioned LXMF attribution projection.

Opaque source hashes project only to sender identity. Human labels come from
text-bearing ``display_name`` and ``short_name`` fields inside ``native.lxmf``.
"""
...
def _project(
    ...
    return project_lxmf_attribution(lxmf_native_data(native_data))
...
def test_project_none_source_hash() -> None:
    ...
    fields = _project({"source_hash": None})
    assert fields["source_sender_id"] is None
...
def test_project_display_name_bytes() -> None:
    ...
    native = {
        "source_hash": "ab" * 16,
        "display_name": "Café".encode("utf-8"),
    }
    fields = _project(native)
...
def test_bytearray_display_name_decoded() -> None:
    ...
    native = {
        "source_hash": "ab" * 16,
        "display_name": bytearray("Café".encode("utf-8")),
    }
    fields = _project(native)
    assert fields["source_sender_label"] == "Café"
