...
# ---------------------------------------------------------------------------
# Inspect event augmentation tests (--timeline, --evidence, --recovery)
# ---------------------------------------------------------------------------
...
def _seed_inspect_failed_db(
    ...
    async def _seed() -> None:
        ...
        # Use msgspec.json to decode with the correct literal type for status.
        receipt_json = {
            "sequence": 1,
        ...
        # Also add a native ref for the event.
...
class TestInspectEventNoFlagsUnchanged:
    ...
    def test_no_flags_returns_event_json(
        ...
        parsed = json.loads(output)
        # Top-level keys are event fields, not nested under "event".
        assert parsed["event_id"] == "evt-inspect-1"
        assert parsed["event_kind"] == "message.created"
        # No augmentation keys present.
...
class TestInspectEventTimeline:
    ...
    def test_timeline_entries_match_trace_event(
        ...
        # Compare timeline entries semantically (same types, same counts).
...
class TestInspectEventRecovery:
    ...
    def test_recovery_runbook_matches_recover_output(
        ...
        # Compare key semantic fields (not formatting).
...
class TestInspectReplay:
    ...
    def test_replay_matches_trace_replay(
        ...
        # Same number of timeline entries with same types.
    ...
    def test_replay_requires_run_id(self) -> None:
        ...
        with pytest.raises(SystemExit):
            main(["inspect", "replay", "--config", "/nonexistent"])
