...
- The ``delivery_outbox.event_id`` foreign key to ``canonical_events(event_id)``
  is declared in the on-disk DDL, enforced at the DB level, and a stamped
  prerelease database missing it is rejected rather than migrated.
"""
...
def _make_outbox(
    ...
    """Build an outbox item with explicit identity for FK tests."""
    return DeliveryOutboxItem(
        outbox_id=outbox_id,
        event_id=event_id,
        route_id=f"route-{delivery_plan_id}",
...
async def test_readonly_connection_sets_busy_timeout(tmp_path: Path) -> None:
    """Readonly readers wait on a busy writer instead of failing instantly."""
    ...
    reader = await SQLiteStorage.open_readonly(db_path)
    try:
        row = await reader._read_one("PRAGMA busy_timeout")
        assert row is not None
        assert int(next(iter(row.values()))) == 5000
    finally:
        await reader.close()
...
async def test_schema_declares_fk_clause(temp_storage: SQLiteStorage) -> None:
...
async def test_existing_outbox_without_fk_is_rejected(tmp_path: Path) -> None:
    ...
    # ``with sqlite3.connect(...)`` only scopes a transaction — the
    # connection must be closed explicitly so no finalizer warning leaks
    # into unrelated tests under filterwarnings=error.
    conn = sqlite3.connect(db_path)
    try:
        conn.execute("PRAGMA foreign_keys=OFF")
        conn.execute("ALTER TABLE delivery_outbox RENAME TO delivery_outbox_current")
        ...
        conn.execute(without_fk)
        conn.execute("DROP TABLE delivery_outbox_current")
        conn.commit()
    finally:
        conn.close()

    storage = SQLiteStorage(str(db_path))
    with pytest.raises(PreReleaseSchemaConstraintMismatchError, match="event_id"):
