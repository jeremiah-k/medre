...
def test_live_route_removed_when_source_and_all_targets_fail(
    tmp_paths: MedrePaths,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """No surviving target wins over a simultaneous source startup failure."""
    db = tmp_path / "combined-start-failure.db"
    config = RuntimeConfig(
        runtime=RuntimeOptions(name="combined-start-failure"),
        logging=LoggingConfig(level="WARNING"),
        storage=StorageConfig(backend="sqlite", path=str(db)),
        adapters=AdapterConfigSet(
            matrix={
                "mx_src": MatrixRuntimeConfig(
            ...
            meshtastic={
                "mesh_tgt": MeshtasticRuntimeConfig(
                    adapter_id="mesh_tgt", enabled=True, adapter_kind="fake"
                ),
                "mesh_unrelated": MeshtasticRuntimeConfig(
                    adapter_id="mesh_unrelated", enabled=True, adapter_kind="fake"
                ),
            },
        ),
        routes=RouteConfigSet(
    ...
    app = RuntimeBuilder(config, tmp_paths).build()
    _patch_start_failure(app.adapters["mx_src"], monkeypatch)
    _patch_start_failure(app.adapters["mesh_tgt"], monkeypatch)
    ...
    async def _scenario() -> None:
        ...
        try:
            ...
            assert app.boot_summary is not None
            assert app.boot_summary.runtime_health == "degraded"
            assert "mesh_unrelated" in app.started_adapter_ids
            ...
            event = _mx_src_event("evt-combined-failure")
            assert app.router.match(event) == [], (
                "LIVE startup retained a route whose source and every target "
                "failed; stored work could dispatch into a dead target"
            )
        finally:
            await app.stop()
...
async def test_worker_activation_failure_cleanup_preserves_external_cancellation(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Cancellation arriving during activation-failure cleanup must win."""
    db = tmp_path / "retry-activation-cancel.db"
    config = _mx_to_mesh_config(
        db,
        retry=RetryConfig(enabled=True, interval_seconds=1.0),
    )
    ...
    async def _blocking_retry_stop(self: Any) -> None:
        cleanup_entered.set()
        await asyncio.Future()

    monkeypatch.setattr("medre.runtime.retry.RetryWorker.start", _fail_retry_start)
    monkeypatch.setattr("medre.runtime.retry.RetryWorker.stop", _blocking_retry_stop)

    start_task = asyncio.create_task(app.start())
    await asyncio.wait_for(cleanup_entered.wait(), timeout=2.0)
    start_task.cancel("external startup cancellation")
