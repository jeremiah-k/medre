...
class TestStatusTransitions:
    ...
    async def test_mark_sent(self, outbox_temp_storage: SQLiteStorage) -> None:
        oid = await self._create_and_claim(outbox_temp_storage, "plan-ts-sent")
        await outbox_temp_storage.mark_outbox_sent(oid, receipt_id="rcpt-sent-1")
    ...
    async def test_mark_queued(self, outbox_temp_storage: SQLiteStorage) -> None:
        oid = await self._create_and_claim(outbox_temp_storage, "plan-ts-queued")
        await outbox_temp_storage.mark_outbox_queued(oid, receipt_id="rcpt-queued-1")
        item = await outbox_temp_storage.get_outbox_item(oid)
        assert item is not None
        assert item.status == "queued"

    async def test_mark_retry_wait(self, outbox_temp_storage: SQLiteStorage) -> None:
        oid = await self._create_and_claim(outbox_temp_storage, "plan-ts-retry")
    ...
    async def test_mark_dead_lettered(self, outbox_temp_storage: SQLiteStorage) -> None:
        oid = await self._create_and_claim(outbox_temp_storage, "plan-ts-dl")
    ...
    async def test_mark_dead_lettered_with_attempt_number(
        ...
        oid = await self._create_and_claim(outbox_temp_storage, "plan-ts-dl-attnum")
        await outbox_temp_storage.mark_outbox_dead_lettered(
            oid,
    ...
    async def test_mark_dead_lettered_without_attempt_number(
        ...
        oid = await self._create_and_claim(
            outbox_temp_storage, "plan-ts-dl-noattnum", attempt_number=3
        )
    ...
    async def test_mark_cancelled(self, outbox_temp_storage: SQLiteStorage) -> None:
        oid = await self._create_and_claim(outbox_temp_storage, "plan-ts-cancel")
        await outbox_temp_storage.mark_outbox_cancelled(oid, error_summary="Shutdown")
        item = await outbox_temp_storage.get_outbox_item(oid)
        assert item is not None
    ...
    async def test_terminal_no_regression(self, outbox_temp_storage: SQLiteStorage) -> None:
        ...
        await outbox_temp_storage.mark_outbox_queued(oid)
        item = await outbox_temp_storage.get_outbox_item(oid)
        assert item is not None
...
class TestStatusTransitionGuards:
    ...
    async def test_pending_cannot_be_marked_sent_directly(
        ...
        oid = await self._create_pending(outbox_temp_storage, "plan-guard-p2s")
        await outbox_temp_storage.mark_outbox_sent(oid)
        item = await outbox_temp_storage.get_outbox_item(oid)
        assert item is not None
    ...
    async def test_retry_wait_cannot_be_marked_queued(
        ...
        oid = await _create_in_progress(outbox_temp_storage, "plan-guard-rw2q")
        await outbox_temp_storage.mark_outbox_retry_wait(
            oid,
    ...
    async def test_queued_can_be_marked_sent(self, outbox_temp_storage: SQLiteStorage) -> None:
        ...
        await outbox_temp_storage.mark_outbox_sent(oid, receipt_id="rcpt-q2s-sent")
        item = await outbox_temp_storage.get_outbox_item(oid)
        assert item is not None
        assert item.status == "sent"
    ...
    async def test_in_progress_can_transition_to_queued(
        ...
        oid = await _create_in_progress(outbox_temp_storage, "plan-guard-ip2q")
        await outbox_temp_storage.mark_outbox_queued(oid)
        item = await outbox_temp_storage.get_outbox_item(oid)
        assert item is not None
        assert item.status == "queued"
    ...
    async def test_in_progress_can_transition_to_sent(
        ...
        oid = await _create_in_progress(outbox_temp_storage, "plan-guard-ip2s")
        await outbox_temp_storage.mark_outbox_sent(oid)
        item = await outbox_temp_storage.get_outbox_item(oid)
        assert item is not None
        assert item.status == "sent"
    ...
    async def test_in_progress_can_transition_to_retry_wait(
        ...
        oid = await _create_in_progress(outbox_temp_storage, "plan-guard-ip2rw")
        await outbox_temp_storage.mark_outbox_retry_wait(
            oid,
            next_attempt_at="2026-01-01T01:00:00",
            failure_kind="adapter_transient",
    ...
    async def test_in_progress_can_transition_to_dead_lettered(
        ...
        oid = await _create_in_progress(outbox_temp_storage, "plan-guard-ip2dl")
        await outbox_temp_storage.mark_outbox_dead_lettered(
    ...
    async def test_terminal_states_cannot_regress(
        ...
        for ts_idx, terminal in enumerate(terminal_statuses):
            ...
            oid = await _create_in_progress(
                outbox_temp_storage, f"plan-guard-term-{terminal}-{ts_idx}"
            )
            if terminal == "sent":
                await outbox_temp_storage.mark_outbox_sent(oid)
                ...
                await outbox_temp_storage.mark_outbox_abandoned(oid)
    ...
    async def test_invalid_transition_leaves_row_unchanged(
        ...
        oid = await self._create_pending(outbox_temp_storage, "plan-guard-noop")
        await outbox_temp_storage.mark_outbox_sent(oid)
        item = await outbox_temp_storage.get_outbox_item(oid)
        assert item is not None
        assert item.status == "pending"
...
class TestQueuedLeaseSemantics:
    ...
    async def test_mark_queued_clears_lease_fields(
        ...
        oid = await _create_in_progress(outbox_temp_storage, "plan-lease-clear")
    ...
    async def test_queued_remains_not_claimable(
        ...
        oid = await _create_in_progress(outbox_temp_storage, "plan-lease-noclaim")
        await outbox_temp_storage.mark_outbox_queued(oid)
    ...
    async def test_queued_to_sent_still_works(
        ...
        lease fields."""
        oid = await _create_in_progress(outbox_temp_storage, "plan-lease-q2s")
        await outbox_temp_storage.mark_outbox_queued(oid)
        await outbox_temp_storage.mark_outbox_sent(oid, receipt_id="rcpt-final")
...
class TestUnknownOutboxStatusRejected:
    ...
    async def test_unknown_status_raises_value_error(
        ...
        with pytest.raises(ValueError, match="Unknown outbox status"):
            await outbox_temp_storage._update_outbox_status(oid, "not_a_real_status")
    ...
    async def test_unknown_status_does_not_mutate_row(
        ...
        before = await outbox_temp_storage.get_outbox_item(oid)
        assert before is not None
        assert before.status == "pending"

        with pytest.raises(ValueError, match="Unknown outbox status"):
            await outbox_temp_storage._update_outbox_status(oid, "bogus_status_xyz")

        after = await outbox_temp_storage.get_outbox_item(oid)
...
class TestNegativeTransitionCases:
    ...
    async def test_queued_cannot_be_marked_retry_wait(
        ...
        oid = await _create_in_progress(outbox_temp_storage, "plan-neg-q2rw")
        await outbox_temp_storage.mark_outbox_queued(oid)
