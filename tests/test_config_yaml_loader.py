...
class TestLoadValidYAML:
    ...
    def test_loads_yml_extension(self, config_file_yml: Path) -> None:
...
class TestLoadRoutes:
    """Route sections parse correctly from YAML."""

    def test_route_parsed(self, routes_config_file: Path) -> None:
...
class TestTOMLRejection:
    """TOML config files must produce a clear, dedicated error."""
    ...
    def test_toml_rejection_exact_message(self, tmp_path: Path) -> None:
    ...
    def test_find_config_rejects_explicit_toml(self, tmp_path: Path) -> None:
        p = tmp_path / "config.toml"
        p.write_text("[runtime]\n")
        with pytest.raises(
            ConfigFileError, match="TOML config files are no longer supported"
    ...
    def test_toml_not_silently_parsed(self, tmp_path: Path) -> None:
...
class TestUnsupportedExtensions:
    """Files with unsupported extensions are rejected at discovery."""
    ...
    def test_json_extension_rejected(self, tmp_path: Path) -> None:
...
class TestLoggingValidationFromYAML:
    """Bad logging config types/levels/formats raise ConfigValidationError."""
    ...
    def test_lowercase_level_canonicalised(self, tmp_path: Path) -> None:
        p = tmp_path / "config.yaml"
        p.write_text("runtime: {}\nlogging:\n  level: debug\n")
