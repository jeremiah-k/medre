...
def _make_lxmf_packet(
    body: str = "hello lxmf", msg_id: str | None = None,
...
class _Setup:
    ...
    __slots__ = ("matrix", "meshtastic", "meshcore", "lxmf", "runner", "accounting", "route_stats")

    def __init__(self, matrix, meshtastic, meshcore, lxmf, runner, accounting, route_stats):
        self.matrix = matrix
...
async def _build_bridge(
    storage: SQLiteStorage, *, prefix: str, routes: list[Route],
) -> _Setup:
    ...
    adapters = {mx_id: fake_matrix, mesh_id: fake_mesh, mc_id: fake_meshcore, lxmf_id: fake_lxmf}

    config = make_pipeline_config(
        storage=storage, router=Router(routes=routes), adapters=adapters,
        rendering_pipeline=rp, accounting=accounting, route_stats=route_stats,
    )
...
class TestConcurrentMatrixAndMeshtasticIngress:
    ...
    async def test_concurrent_matrix_and_meshtastic_ingress(
        ...
        try:
            mx_coros = [
                s.matrix.simulate_inbound(s.matrix.make_event(
...
class TestConcurrentMixedAdapterIngress:
    ...
    async def test_concurrent_mixed_adapter_ingress(
        ...
        routes = [
            Route(id=f"{p}-mx-to-mesh",
                  source=RouteSource(adapter=mx_id, event_kinds=("message.created",), channel=None),
                  targets=[RouteTarget(adapter=mesh_id, channel="0")]),
            Route(id=f"{p}-mesh-to-mx",
                  source=RouteSource(adapter=mesh_id, event_kinds=("message.created",), channel=None),
                  targets=[RouteTarget(adapter=mx_id, channel=f"!{p}:fake")]),
            Route(id=f"{p}-mc-to-mx",
                  source=RouteSource(adapter=mc_id, event_kinds=("message.created",), channel=None),
                  targets=[RouteTarget(adapter=mx_id, channel=f"!{p}:fake")]),
            Route(id=f"{p}-lxmf-to-mx",
                  source=RouteSource(adapter=lxmf_id, event_kinds=("message.created",), channel=None),
                  targets=[RouteTarget(adapter=mx_id, channel=f"!{p}:fake")]),
        ]
        ...
        try:
            coros = []
            for i in range(2):
                coros.append(s.matrix.simulate_inbound(s.matrix.make_event(
                    text=f"t2 mx {i}", event_kind=EventKind.MESSAGE_CREATED)))
                coros.append(s.meshtastic.simulate_inbound(
                    make_text_packet(text=f"t2 mesh {i}", packet_id=2000 + i)))
                coros.append(s.meshcore.simulate_inbound(
                    make_meshcore_packet(text=f"t2 mc {i}", packet_id=3000 + i)))
                coros.append(s.lxmf.simulate_inbound(_make_lxmf_packet(
                    body=f"t2 lxmf {i}", msg_id=f"{'ff'*15}{i:02x}" * 2,
...
class TestConcurrentIngressCleanShutdown:
    ...
    async def test_shutdown_with_capacity_controller(
        ...
        cc = CapacityController(RuntimeLimits(
            max_inflight_deliveries=2, max_inflight_replay_events=2,
    ...
    async def test_no_semaphore_leak_after_stress(
        ...
        cc = CapacityController(RuntimeLimits(
            max_inflight_deliveries=5, max_inflight_replay_events=5,
