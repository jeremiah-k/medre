...
class TestClaimDueItems:
    ...
    async def test_claim_pending_item(self, outbox_temp_storage: SQLiteStorage) -> None:
    ...
    async def test_claim_does_not_double_claim(
        ...
        worker2 = await outbox_temp_storage.claim_due_outbox_items(
...
class TestReleaseClaim:
    ...
    async def test_release_claim_restores_pending(
        ...
        released = await outbox_temp_storage.get_outbox_item(oid)
    ...
    async def test_release_wrong_worker_noop(self, outbox_temp_storage: SQLiteStorage) -> None:
        ...
        await outbox_temp_storage.release_outbox_claim(oid, "worker-2")
    ...
    async def test_release_non_in_progress_is_noop(
        ...
        after = await outbox_temp_storage.get_outbox_item(created.outbox_id)
    ...
    async def test_release_retry_wait_item_is_noop(
        ...
        item_rw = await outbox_temp_storage.get_outbox_item(oid)
        assert item_rw is not None
        assert item_rw.status == "retry_wait"

        # Attempt to release — should be a no-op
        await outbox_temp_storage.release_outbox_claim(oid, "worker-1")
...
class TestClaimClearsNextAttemptAt:
    ...
    async def test_claim_clears_next_attempt_at_from_retry_wait(
        ...
        stored = await outbox_temp_storage.get_outbox_item(claimed[0].outbox_id)
