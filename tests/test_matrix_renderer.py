...
class TestMultiRadioSourceConfig:
    ...
    def _source_attribution() -> dict[str, _StubSourceAttribution]:
        """source_attribution registry for KEY_MESHNET (populated from origin_label)."""
        return {
            ...
            "radio-bravo": _StubSourceAttribution(
                adapter_id="radio-bravo",
                origin_label="BravoNet",
            ),
        }

    def _make_renderer(self, **kwargs: object) -> MatrixRenderer:
...
class TestRuntimeAssemblySourceConfig:
    ...
    Runtime assembly passes ``source_configs`` and ``source_attribution``.
    Unknown / non-Meshtastic sources render plain Matrix output without
...
class TestResolveMmrelaySenderNames:
    """``_resolve_mmrelay_sender_names`` resolution order per field.
    ...
    def test_namespaced_longname_is_primary(self) -> None:
    ...
    def test_namespaced_shortname_is_primary(self) -> None:
    ...
    def test_namespaced_wins_over_bare(self) -> None:
    ...
    def test_wire_key_resolves_when_no_namespaced(self) -> None:
    ...
    def test_key_constant_resolves(self) -> None:
    ...
    def test_bare_key_legacy_tolerance(self) -> None:
    ...
    def test_namespaced_wins_over_wire(self) -> None:
    ...
    def test_wire_wins_over_bare(self) -> None:
    ...
    def test_empty_when_no_identity_keys(self) -> None:
        """No identity keys anywhere yields empty strings."""
        longname, shortname = MatrixRenderer._resolve_mmrelay_sender_names({})
        assert longname == ""
        assert shortname == ""

    def test_displayname_does_not_leak(self) -> None:
    ...
    def test_longname_and_shortname_resolve_independently(self) -> None:
