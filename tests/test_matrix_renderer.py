...
# ---------------------------------------------------------------------------
# Native-target selection rules: relations[0], multiple, stale, no pre-validation
# ---------------------------------------------------------------------------


class TestMatrixTargetSelectionRules:
    """Lock the target-selection contracts for MatrixRenderer.

    These tests assert the *current* behaviour — they are guards against
    accidental changes, not aspirational specifications.

    Key contracts:
    - MatrixRenderer inspects **only** ``event.relations[0]``.
    - Native target ID comes from ``target_native_ref.native_message_id``
      owned by ``target_adapter`` — never from ``target_event_id``.
    - No staleness or liveness check is performed on the native target.
    - When ``target_native_ref`` is ``None`` or foreign, native rendering
      is suppressed and an appropriate fallback is used instead.
    """

    async def test_uses_relations_zero_only_for_reply(self) -> None:
        """When multiple relations exist, only relations[0] is considered.

        A reply at index 0 is rendered; a reaction at index 1 is ignored.
        """
        renderer = MatrixRenderer()
        reply_rel = EventRelation(
            ...
            target_native_ref=NativeRef(
                adapter="matrix-1",
                native_channel_id="!room:server",
                native_message_id="$reply-target",
            ),
            key=None,
            fallback_text="original",
        )
        ...
        # Reply is rendered natively
        assert "m.relates_to" in result.payload
        assert "m.in_reply_to" in result.payload["m.relates_to"]
        # No reaction artifacts (no emoji in m.relates_to)
        assert result.payload["m.relates_to"].get("rel_type") != "m.annotation"

    async def test_uses_relations_zero_only_for_reaction(self) -> None:
        ...
        reaction_rel = EventRelation(
            ...
            target_native_ref=NativeRef(
                adapter="matrix-1",
                native_channel_id="!room:server",
                native_message_id="$react-target",
            ),
            key="❤️",
            fallback_text=None,
        )
        ...
        # Reaction is rendered natively
        assert result.payload["_matrix_event_type"] == "m.reaction"
        assert result.payload["m.relates_to"]["key"] == "❤️"
        # No reply artifact (m.in_reply_to)
        assert "m.in_reply_to" not in result.payload["m.relates_to"]
    ...
    async def test_stale_native_target_used_without_prevalidation(self) -> None:
        ...
        MatrixRenderer performs no staleness or liveness check on the
        native_message_id.  It passes the ID through to m.relates_to
        verbatim — the homeserver is responsible for rejecting unknown
        event IDs.
        """
        renderer = MatrixRenderer()
        stale_ref = NativeRef(
            adapter="matrix-1",
            native_channel_id="!room:server",
            native_message_id="$possibly-stale-event-id",
        )
        rel = EventRelation(
            relation_type="reply",
            target_event_id="orig-001",
        ...
        # The potentially-stale ID is emitted without any pre-validation
        assert result.payload["m.relates_to"]["m.in_reply_to"]["event_id"] == (
            "$possibly-stale-event-id"
        )

    async def test_target_event_id_never_used_as_matrix_id(self) -> None:
        """The canonical target_event_id is never used as a Matrix event ID.

        Even when target_native_ref is None, target_event_id must NOT appear
        in m.relates_to.  Only native_message_id from a matching adapter
        qualifies.
        """
        renderer = MatrixRenderer()
        rel = EventRelation(
            relation_type="reply",
            target_event_id="canonical-uuid-abc123",
            target_native_ref=None,
            key=None,
            fallback_text="original",
        )
        ...
        # No m.relates_to at all — no native ref to use
        assert "m.relates_to" not in result.payload
        # Canonical ID must not appear anywhere in the payload
        assert "canonical-uuid-abc123" not in str(result.payload)
