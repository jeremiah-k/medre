...
class TestMeshtasticAdapterQueueOwnership:
    ...
    async def test_send_one_dequeues_and_sends_with_fake_client(
        ...
        assert result.handoff.native_message_id == "77"
...
class TestMeshtasticAdapterClassifierCounters:
    ...
    async def test_encrypted_increments_sub_counter(self, make_adapter_context) -> None:
        ...
        try:
            ...
            diag = adapter.diagnostics()
            assert diag["classifier_packets_dropped"] == 1
            assert diag["classifier_packets_encrypted_dropped"] == 1
        finally:
            await adapter.stop()
    ...
    async def test_detection_sensor_increments_deferred_counter(
        ...
        try:
            ...
            assert diag["classifier_packets_detection_sensor_deferred"] == 1
        finally:
            await adapter.stop()
    ...
    async def test_unknown_portnum_increments_unknown_counter(
        ...
        try:
            ...
            diag = adapter.diagnostics()
            assert diag["classifier_packets_deferred"] == 1
            assert diag["classifier_packets_unknown_portnum_deferred"] == 1
        finally:
            await adapter.stop()
