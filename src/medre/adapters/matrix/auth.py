...
def update_toml_access_token(
    ...
    # Pattern: access_token = "..." or access_token='...'
    token_pattern = re.compile(
        r"^(\s*access_token\s*=\s*)\"[^\"]*\"(.*)$"
    )
    token_pattern_sq = re.compile(
        r"^(\s*access_token\s*=\s*)'[^']*'(.*)$"
    )
    ...
    for line in lines:
        ...
        if in_target_section and not found_key:
            ...
            if not m:
                m = token_pattern_sq.match(line)
            if m:
                line = f'{m.group(1)}"{_toml_escape_string(token)}"{m.group(2)}\n'
                found_key = True
    ...
    if not found_key:
        ...
        section_found = any(
            l.strip() == section_header for l in lines
        )
