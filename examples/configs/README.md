...
| `mixed-matrix-meshtastic.yaml`            | Earlier Matrix ↔ Meshtastic bridge variant         | Matrix + Meshtastic                         | Yes (homeserver + radio)     | Retained for backward compatibility; prefer `live-matrix-meshtastic.yaml` for new setups.                                                                          |
...
| `lxmf-receiver.yaml`                      | Minimal LXMF receiver template                     | LXMF                                        | Yes (LXMF hardware)          | Minimal template — LXMF receiver, requires `storage_path` via env                                                                                                  |
| `lxmf-sender.yaml`                        | Minimal LXMF sender template                       | LXMF                                        | Yes (LXMF hardware)          | Minimal template — LXMF sender, requires `storage_path` via env                                                                                                    |
| `meshcore-lab.yaml`                       | Minimal MeshCore TCP template                      | MeshCore                                    | Yes (MeshCore TCP)           | Minimal template — MeshCore TCP, requires `host` via env                                                                                                           |
| `meshcore-tbeam.yaml`                     | Minimal MeshCore BLE template                      | MeshCore                                    | Yes (MeshCore BLE)           | Minimal template — MeshCore BLE, requires `ble_address` via env                                                                                                    |
