...
- Added a transport-neutral, append-only `delivery_observations` ledger for facts
  ...
  prior-attempt callback is admitted and a live next-attempt callback waits for
  finalization (documented in `docs/spec/routing-delivery.md` §13.3.1).
...
- LXMF now carries opaque attempt correlation through its asynchronous delivery
  ...
  not synthesize unreported terminal states. Immediate handoff remains
  `sent/local_queue`.
