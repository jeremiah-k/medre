...
- **Test coverage**: Matrix has live smoke evidence against Synapse. Meshtastic has Docker SDK-boundary evidence (containerized meshtasticd) only. MeshCore and LXMF are unit-tested only (fake-pipeline). Both have fully implemented inbound ingress paths (SDK callback → codec → publish_inbound) but no live hardware validation.
