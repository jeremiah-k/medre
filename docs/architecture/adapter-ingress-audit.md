...
- **Why xfail and not skip.** The test is `xfail(strict=False)` rather than
  `skip` because it provides *bonus evidence* when it passes — some
...
*client library* that connects to real MeshCore radio hardware via TCP,
...
3. The SDK itself supports async operation and would work in a containerized
   test environment *if* there were something to connect to.
