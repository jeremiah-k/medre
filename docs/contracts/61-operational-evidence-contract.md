...
| `status`             | string          | Yes      | Final delivery receipt status: `accepted`, `queued`, `sent`, `confirmed`, `suppressed`, `failed`, `dead_lettered`. The `suppressed` status covers loop/capacity/shutdown/policy rejection receipts persisted where event/target context exists. |
...
| `policy_suppressed`      | Visible in `RouteStats.policy_suppressed` when the route-policy evaluator denies a delivery. Delivery outcomes carry `failure_kind=POLICY_SUPPRESSED`. The pipeline persists a `status="suppressed"` receipt with the policy denial reason code (one of: `source_adapter_not_allowed`, `dest_adapter_not_allowed`, `sender_not_allowed`, `room_not_allowed`, `channel_not_allowed`), `route_id`, `target_adapter`, and `target_channel`. Policy suppression is permanent and not retryable. |
...
| `suppressed_count`          | Count of persisted receipts with `status="suppressed"` (covers loop_suppressed, policy_suppressed). Non-persisted suppressions (capacity_rejection, shutdown_rejection) are counted separately from the receipt pool.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `sent_unconfirmed_count`    | Count of `sent` receipts (not yet confirmed by transport).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `delivery_state_by_target`  | Per-target delivery state dict. Keys are JSON strings containing `delivery_plan_id`, `route_id`, `target_adapter`, `target_channel` (sorted keys, via `json.dumps(..., sort_keys=True)`). Each value includes decomposed fields: `target_adapter`, `target_channel`, `route_id`, `delivery_plan_id`, `status`, `attempt_number`, `failure_kind`, `failure_kind_detail`, `retryable`, `next_retry_at`, `native_message_id`, `adapter_message_id`. Each entry selects the receipt with the highest `attempt_number` for that composite key, then the latest receipt sequence within that attempt. Same adapter on multiple channels or same channel via different routes produces separate entries, so no target is hidden by adapter-level collapse. |
...
4. **`duplicate_suppressed` has been removed from the enum.** The runtime suppresses duplicates at ingress without creating a receipt. No `DUPLICATE_SUPPRESSED` failure kind exists. Duplicate suppression returns an empty outcomes list before storage.
...
| 2026-05-24 | v7      | Replaced `delivery_state_by_adapter` with `delivery_state_by_target` in incident summary (§3.8.5). Keys are composite JSON strings containing `delivery_plan_id`, `route_id`, `target_adapter`, `target_channel`. Values include all decomposed fields from the highest-attempt receipt per target. Same adapter on multiple channels or same channel via different routes now appears as separate target entries, preventing adapter-level collapse from hiding failed or dead-lettered deliveries. Removed adapter-keyed dict from output.                                                                                                                                                                          |
