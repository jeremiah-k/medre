...
- Adapter-local outbound queue durability. Items remaining in an adapter's in-memory outbound queue at process termination (graceful or ungraceful) are lost. The Meshtastic adapter's outbound queue is non-durable; however, the `delivery_outbox` table provides durable tracking for in-flight and retry-waiting deliveries, and the RetryWorker can re-claim outbox items with expired leases after restart (see Contract 59 §3). Adapter-local queue contents are still lost on crash — only the outbox row survives.
