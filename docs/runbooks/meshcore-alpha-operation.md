...
| Backlog suppression                  | Deferred (should not be wired)                            | MeshCore has no store-and-forward/initial sync; `sender_timestamp` is sender-side/unverified. Suppressing live packets risks dropping fresh traffic. Revisit if MeshCore gains backlog semantics |
