...
3. **Record the reason.** Every **NOT EXECUTED** entry must include:
   ...
   - What command _would_ be run (e.g., `pytest tests/test_meshcore_live.py -m live -v`)
   - What environment variables _would_ be needed
...
| G5  | In-flight work lost on crash (partially recoverable via outbox) | By design + outbox lease-reclaim tests (`test_outbox_race_model`, `test_storage_outbox`); the "no outbox row = fully lost" aspect is documented by design | Contract 59 §3.3, §4.1 |
...
All G1–G8 must be verified by passing tests or documented design rationale. G5 is partially verified by outbox reclaim tests; the "no outbox row = fully lost" aspect is documented by design.
...
3. `test_packaging_and_install_contract.py::TestPackageMetadata::test_classifiers_include_alpha` — expects `"Development Status :: 3 - Alpha"`; NOT EXECUTED in clean venv (requires full install with optional SDKs). The Alpha classifier in `pyproject.toml` is correct — the project remains pre-release.
...
| LB4 | Matrix adapter live      | `pytest tests/test_live_matrix_meshtastic_bridge.py -m live -k "matrix_adapter_healthy"`                               | PASS (requires MATRIX\_\* env vars)                                   |
| LB5 | Meshtastic adapter live  | `pytest tests/test_live_matrix_meshtastic_bridge.py -m live -k "meshtastic_adapter_healthy"`                           | PASS (requires MESHTASTIC\_\* env vars)                               |
