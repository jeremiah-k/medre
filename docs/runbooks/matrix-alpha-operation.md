...
Before running live validation, capture the exact environment:
...
# E2EE dependencies (if testing encrypted rooms)
pip show vodozemac 2>/dev/null || echo "NOT INSTALLED (E2EE)"
pip show peewee 2>/dev/null || echo "NOT INSTALLED"

# Homeserver reachability (non-secret)
curl -s http://localhost:8008/_matrix/client/versions 2>/dev/null | python3 -m json.tool
```
...
| ---------------------------- | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| End-to-end encryption (E2EE) | E2EE text alpha available (section 15) | Encrypted rooms are supported for text messages when installed with `.[matrix-e2e]` and `encryption_mode` is set to `e2ee_required` or `e2ee_optional`. Device ID is discovered via `whoami()` and store path is derived internally. True Matrix reactions (`m.reaction` event type) are limited in encrypted rooms because mindroom-nio does not encrypt `message_type="m.reaction"` events; text messages and MMRelay emote fallback remain the safe encrypted paths. Edits, media, cross-signing, key backup, and unverified device policy remain unsupported. Undecryptable event logging is now implemented (counted, logged, not forwarded). Plaintext rooms work identically in both modes. |
| Reactions                    | Implemented (Tranche 3), encrypted-room limitation | Matrix-native `m.annotation` reaction delivery and decoding were implemented and verified in Tranche 3. The codec extracts true reactions and MMRelay emote reactions; the renderer produces `m.reaction` content. **Limitation:** mindroom-nio's `room_send` does not encrypt `message_type="m.reaction"` events, so true Matrix reactions are sent in cleartext even in encrypted rooms. The MMRelay emote fallback path uses `message_type="m.room.message"` with `msgtype="m.emote"`, which IS encrypted in encrypted rooms and remains the recommended path for encrypted-room reaction-like behavior. |
