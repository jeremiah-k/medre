...
**Shutdown queue abandonment — partial recovery:** Items remaining in the adapter-local outbound queue at shutdown are **not** persisted to the adapter queue itself and are lost. However, the `delivery_outbox` table (see Contract 59 §3) provides durable tracking: outbox rows with status `in_progress` and an expired lease, or status `retry_wait`/`pending`, survive the crash and are re-claimable by the RetryWorker on restart (when `[retry] enabled = true`). Stale `queued` rows past the `STALE_QUEUED_GRACE_SECONDS` threshold (default 300 s) are also reclaimed. Adapter-local queue contents are in-memory and non-durable; outbox state is persisted to SQLite. Operators should ensure the queue is drained before shutdown for cleanest recovery.
