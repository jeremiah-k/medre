...
**Shutdown queue abandonment — partial recovery:**

- **Adapter-local queue:** Items remaining in the adapter-local outbound queue at shutdown are **not** persisted and are lost. The queue is in-memory and non-durable.
- **Durable outbox tracking:** The `delivery_outbox` table (see Contract 59 §3) provides durable tracking via SQLite. Outbox rows with status `in_progress` and an expired lease, or status `retry_wait`/`pending`, survive the crash and are re-claimable by the RetryWorker on restart (when `[retry] enabled = true`). Stale `queued` rows past the `STALE_QUEUED_GRACE_SECONDS` threshold (default 300 s) are also reclaimed.
- **Recommendation:** Ensure the adapter-local queue is fully drained before shutdown for cleanest recovery.

**Duplicate-send caveat:** Reclaiming expired `in_progress` or stale `queued` outbox rows after a crash/restart may produce duplicate sends. The adapter may have already delivered the message before the crash, but no receipt was committed. The RetryWorker cannot distinguish completed-but-unreceipted deliveries from genuinely failed deliveries — reclaimed rows are re-attempted as new work. Operators should inspect the Meshtastic node's sent-message history when investigating recovered deliveries.
...
13. **Packet classifier numeric map uses SDK-derived resolution when available.** When the `mtjk` package is installed, numeric portnum values are resolved via `compat.get_portnum_table()` which reads the real protobuf `PortNum` enum. When `mtjk` is not installed, a protocol-correct fallback map (`_NUMERIC_PORTNUM_FALLBACK`) is used. All included fallback values match their authoritative protobuf names exactly; numeric values not present in the fallback are returned as strings and classified as unknown/deferred. See `docs/contracts/10-meshtastic-source-audit.md` for the full fallback table.
...
| Shutdown queue durability    | **Non-guarantee (partial recovery)**      | Adapter-local outbound queue items remaining at shutdown are lost, but durable `delivery_outbox` rows can be reclaimed by RetryWorker after restart (when retry is enabled). See section 6.3 |
