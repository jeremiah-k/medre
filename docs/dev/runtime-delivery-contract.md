...
- **Native ref dedup**: during `handle_ingress`, if `event.source_native_ref` has a `native_message_id`, the pipeline calls `storage.resolve_native_ref()`. If the triple `(adapter, native_channel_id, native_message_id)` already maps to a stored event, the new event is suppressed
