...
- **Queue state**: `queue_total_enqueued`, `queue_total_sent`, `queue_total_failed`, `queue_total_rejected`, `queue_total_requeued`, `queue_total_exhausted`, `queue_total_permanent_failed`, `queue_send_max_attempts`, `queue_pending`, `queue_max_size`. Being **queued**, **enqueued**, or **sent** means the local node accepted the packet into its outbound queue and sent it to the radio. This is **not RF confirmation** — there is no acknowledgment from any remote node that the packet was received over the air. Confirmed/ack semantics remain distinct and are not currently available from the Meshtastic adapter. Transient SDK send failures are retried from the adapter-local queue up to `queue_send_max_attempts`; permanent failures and exhausted retries are dropped. Retry is best-effort, adapter-local, in-memory, non-durable across process restart, and not exactly-once.
