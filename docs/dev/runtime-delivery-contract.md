...
| target_adapter    | str          | Target adapter name                                                                                                 |
| target_channel    | str or None  | Target channel                                                                                                      |
| target_address    | str or None  | Transport-specific direct target address, such as a DM recipient/native peer ID; null for channel-based deliveries. |
| attempt_number    | int          | 1-indexed attempt counter                                                                                           |
| status            | str          | pending/in_progress/queued/sent/retry_wait/dead_lettered/cancelled/abandoned                                        |
| failure_kind      | str or None  | Failure classification                                                                                              |
...
- Live pipeline creates items as `in_progress` with a lease (locked_at, lease_until, worker_id).
- The lease prevents the RetryWorker from claiming live work (unexpired in_progress items are not claimable).
- On delivery success: transitions to `sent` (terminal).
- On adapter-queue acceptance (Meshtastic): transitions to `queued`; adapter-local queue callback later transitions `queued` → `sent`.
- On transient failure: transitions to `retry_wait` with `next_attempt_at` set via exponential backoff.
- On permanent failure or retry exhaustion: transitions to `dead_lettered` (terminal).
- Expired `in_progress` items (lease_until <= now) can be reclaimed by the RetryWorker after crash/restart.
- Terminal states (`sent`, `dead_lettered`, `cancelled`, `abandoned`) cannot regress.
...
- **RetryWorker** (src/medre/runtime/retry.py): background asyncio task polling `claim_due_outbox_items` at configurable interval (default 10s). The RetryWorker processes outbox items only — receipts remain the evidence/audit trail; the outbox is operational work state.
...
- **Shutdown queue abandonment**: Items remaining in the Meshtastic adapter-local outbound queue at process termination are lost. The adapter-local queue is in-memory and non-durable. However, the `delivery_outbox` table provides durable tracking: outbox rows with status `in_progress` and an expired lease, or status `retry_wait`/`pending`, survive the crash and are re-claimable by the RetryWorker on restart. Delivery receipts already written to SQLite also survive shutdown. Only the in-memory adapter queue contents are lost.
...
| `suppressed_count`         | Count of persisted receipts with `status="suppressed"` (covers loop_suppressed, policy_suppressed). `CAPACITY_REJECTION` and `SHUTDOWN_REJECTION` produce lightweight persisted suppression receipts (not adapter delivery receipts). Duplicate suppression produces no receipt.                                                                                                                                                                                                                                                                                                                               |
