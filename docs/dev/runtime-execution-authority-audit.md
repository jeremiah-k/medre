...
CLI (run_commands.py)
  ...
  └─ MedreApp (runtime/app.py)
       ...
       │    ├─ DeliveryLifecycleService (core/engine/pipeline/delivery_lifecycle.py)
       │    │    Owns durable delivery/retry lifecycle decisions, attempt
       │    │    evidence correlation, retry classification, and transitions.
       │    └─ TargetDeliveryService (core/engine/pipeline/target_delivery.py)
       │         Owns per-target execution: rendering, adapter invoke, primary
       │         receipt construction, and primary-attempt failure evidence.
       ├─ RetryWorker (runtime/retry.py)
       │    Owns retry polling/orchestration: claim due outbox items,
       │    reconstruct delivery context, call pipeline, capacity, counters,
       │    runtime events. Delegates durable retry classification/transitions
       │    to the shared DeliveryLifecycleService.
...
| RetryWorker                           | `runtime/retry.py`                               | `MedreApp`                                          | `app.start()` step 2.5 (if `config.retry.enabled` and storage present) | `app.stop()` Phase 1                                  | None directly; durable retry/outbox mutations are delegated to the shared `DeliveryLifecycleService` | `claim_due_outbox_items`, `get()`, `delivery_status()`, `count_outbox_by_status()` | Two-stage: cooperative (shutdown event + poll), then forced `cancel()` + poll. Abandonment if task survives both grace periods | `stop()` bounded by `stop_timeout_seconds`. The RetryWorker may be **abandoned**: `state.abandoned=True`, `state.running=False`, retained abandoned task may still be alive. `state.running` describes managed RetryWorker lifecycle (no active `_task`). `abandoned=True` blocks restart until explicit state reset. | Claims due outbox rows; lifecycle service preflight-reconciles persisted next-attempt evidence before transport, classifies failure/acceptance, and commits retry/dead-letter/success transitions | RetryWorkerState in snapshot, `retry_started/stopped/failed/abandoned` events, `retry_start_refused` event on abandoned restart | Best-effort: abandonment is a known outcome. Abandoned state blocks `start()` until operator intervention                                                                                                       |
...
| Delayed native-ref queue (Meshtastic) | `adapters/meshtastic/adapter.py`                 | `MeshtasticAdapter`                                 | `_drain_task` started in `adapter.start()`                             | `adapter.stop()` cancels drain task                   | Via `PipelineRunner._record_outbound_native_ref()` callback                                                                           | None                                                                                    | `cancel()` on drain task                                                                                                       | `stop()` cancels drain, awaits with timeout                                                                                                                                                                                                                                                           | Not involved (adapter-local queue)                                                                                               | Supplemental `sent` receipt bridges `queued` to `sent` evidence gap                                                             | Best-effort: items abandoned on drain task cancellation by design                                                                                                                                               |
