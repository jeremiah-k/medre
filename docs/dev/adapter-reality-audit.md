...
| Reference                     | Path / ID                                                              | Version          | What was checked                                                                                                         |
| ----------------------------- | ---------------------------------------------------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------ |
| mmrelay                       | `/home/jeremiah/dev/meshtastic-matrix-relay/`                          | commit `7b9efca` | Meshtastic packet handling, relay logic, self-echo filtering                                                             |
| meshtastic-python (mtjk)      | `/home/jeremiah/dev/meshtastic-python/`                                | v2.5.10          | SDK API shapes, `sendText()` return, `myInfo.myNodeNum`, classifier portnum map                                          |
| Meshtastic firmware protobufs | `/home/jeremiah/dev/meshtastic/protobufs/meshtastic/`                  | current          | `PortNum` enum values, `Data.reply_id`, `Data.emoji`, packet field presence                                              |
| mindroom-nio                  | `/home/jeremiah/dev/mindroom-nio/src/nio/`                             | current          | Matrix send responses, `event_id` presence, error code strings (`M_FORBIDDEN`, `M_NOT_FOUND`, `M_DUPLICATE_ANNOTATION`)  |
| Matrix client-server spec     | `/home/jeremiah/dev/matrix-spec/` + Context7 `/matrix-org/matrix-spec` | v1.14            | `txn_id` semantics, annotation endpoint errors, `event_id` vs `M_UNKNOWN` edge cases                                     |
| meshcore_py                   | `/home/jeremiah/dev/meshcore/meshcore_py/src/meshcore/`                | v2.2.5           | `send_msg()` return shape (`expected_ack`, `suggested_timeout`), `send_appstart()` requirement, `MSG_SENT` event payload |
| MeshCore firmware             | `/home/jeremiah/dev/meshcore/MeshCore/`                                | current          | APP_START command requirement after connect, ACK protocol docs                                                           |
| LXMF                          | `/home/jeremiah/dev/LXMF/LXMF/` + Context7 `/markqvist/lxmf`           | v0.9.6           | `LXMessage` state model, delivery state progression, `RNS.Identity.recall()` vs self-identity destination                |
| Reticulum docs                | <https://reticulum.network/manual/reference.html>                      | current          | `RNS.Destination` constructor semantics, identity recall API                                                             |
