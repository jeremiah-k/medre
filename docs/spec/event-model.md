...
| `root_event_id`       | `str \| None`               | `None`  | Canonical event ID of the root event in a relation chain. `None` when not yet computed or when the event has no relation ancestry. Populated by `ConversationGraphAuthority` at pipeline Stage 2.5.           |
| `conversation_id`     | `str \| None`               | `None`  | Canonical conversation identifier. Expected to equal `root_event_id` for now, but the field exists independently to allow future divergence. Populated by `ConversationGraphAuthority` at pipeline Stage 2.5. |
