...
| `delivery_status`    | Adapter-level delivery state: `"sent"` (default, synchronous adapters) or `"enqueued"` (queue-based adapters that accepted locally but have not yet sent to the platform). Transport-specific local-acceptance states (e.g., MeshCore's `local_accepted`) are reported in `metadata.adapter_status`, **not** in this field. |
| `metadata`           | Immutable, namespaced delivery metadata. Transport-specific data **MUST** live under `metadata[<transport>]`. A small set of MEDRE-standard top-level keys (`adapter_status`) is permitted for cross-transport status evidence; no other loose ad-hoc fields.                                                               |
