...
| `delivery_status`    | Adapter-level delivery state: `"sent"` (default, synchronous adapters) or `"enqueued"` (queue-based adapters that accepted locally but have not yet sent to the platform). Transport-specific local-acceptance states are reported in `metadata[<transport>]`, **not** in this field. |
