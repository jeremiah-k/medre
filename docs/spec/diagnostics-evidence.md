...
| `convergence_summary`          | `dict or None` | Per-event convergence diagnostics summary derived from delivery receipts and outbox items (see § 21). When collected with an `event_id`, per-event; without, the global view across all delivery targets. `None` when the storage section is absent, the database is empty, or no data matches the scoped query. |
| `orphan_report`                | `dict or None` | Per-event orphan/invalid-lineage report derived from delivery receipts and outbox items (see § 21.8). When collected with an `event_id`, per-event; without, the global view across all delivery targets. `None` when the storage section is absent, the database is empty, or no data matches the scoped query. |
