...
| `convergence_summary`          | `dict or None` | Convergence diagnostics summary derived from delivery receipts and outbox items (see § 21). When collected with an `event_id`, event-scoped; without, the global view across all delivery targets. Global convergence is bounded at 10 000 receipts and 10 000 outbox rows; when the cap is reached the storage section status is `"partial"` and data includes `convergence_truncated_warning`. `None` when the storage section is absent, the database is empty, or no data matches the scoped query. |
| `orphan_report`                | `dict or None` | Orphan/invalid-lineage report derived from delivery receipts and outbox items (see § 21.8). When collected with an `event_id`, event-scoped; without, the global view across all delivery targets. `None` when the storage section is absent, the database is empty, or no data matches the scoped query.                                                                                                                                                                                               |
...
| `lifecycle_convergence_report` | `dict or None` | Lifecycle delivery convergence findings (receipt/outbox mismatches, retry anomalies, stalled plans) (see § 23). When collected with an `event_id`, event-scoped; without, the global view across all delivery targets. `None` when the storage section is absent, the database is empty, or no data matches the scoped query.                                                                                                                                                                           |

The five diagnostics fields fall into two groups with different scoping rules:

- **Event-scoped (storage-derived):** `convergence_summary`, `orphan_report`, `lifecycle_convergence_report` are hoisted from `sections.storage.data`. When the bundle is collected with an `event_id`, the hoisted values are per-event (a single target set, single event's lineage). When collected without an `event_id` (e.g. `medre evidence` without `--event`), they carry the global view across all delivery targets in the storage backend. They are `None` when the storage section is absent or could not be collected (e.g. config-error path, missing database), or when `_collect_storage_data_from_backend()` finds no relevant receipts/outbox items for the scoped query (e.g. empty database, non-matching event_id). Global convergence queries are bounded at 10 000 receipts and 10 000 outbox rows; when either cap is reached the storage section reports `"partial"` status and includes a `convergence_truncated_warning` string in the section data. Capped global evidence MUST be treated as incomplete.

- **Startup-scoped (recovery-derived):** `recovery_summary`, `recovery_ledger` are hoisted from `sections.recovery.data`. These describe the outcome of the most recent startup recovery pass — they are not event-scoped. They are `None` when the recovery section is absent (e.g. config-error path or storage-only mode) or when `sections.recovery.data` is present but skipped/empty (e.g. a skipped backend).
...
3. `delivery_plan_id` is validated against the outbox item's `delivery_plan_id` when present. A mismatch causes the callback to be rejected. When absent, correlation proceeds via `outbox_id` but validation is skipped.
4. All ambiguous correlation skips, missing `outbox_id` hard-rejections, and missing `delivery_plan_id` validation skips MUST log at warning level. Ordinary no-match situations (no candidates at all) MAY remain at debug level. Warning messages MUST include event_id, adapter, outbox_id, delivery_plan_id if available, native_channel_id if available, candidate count, and distinct plan/channel counts where useful.
