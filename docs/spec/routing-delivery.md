...
1. `deliver_to_target` records append-only attempt evidence. A retryable adapter
   ...
   normal retry-policy backoff and that hint. The adapter-provided hint
...
10. A non-retryable current-attempt failure is dead-lettered immediately. A retryable failure at `attempt_number >= max_attempts` is dead-lettered as retry exhaustion. Attempt failure and lifecycle terminalization keep the same dispatch attempt number: `failed` records the execution result, while linked `dead_lettered` lifecycle evidence records the state transition. For outbox-backed terminalization, lifecycle authority is inserted atomically with the guarded terminal outbox transition rather than pre-appended. Missing or invalid taxonomy on persisted `failed` retry evidence is an invariant violation; reconciliation terminally repairs the outbox as `adapter_permanent` rather than leaving it indefinitely reclaimable.
...
the effective retry delay beyond that cap. The adapter-provided hint contribution
is clamped to a 30-day scheduling maximum; it MUST NOT shorten policy backoff,
bypass retry exhaustion, or make a non-retryable failure retryable.
...
| **Reconnect model**                 | Lifetime retry, capped 1–60 s backoff; mindroom-nio timeout retry disabled so MEDRE owns restart | Lifetime MEDRE client recreation, capped 1–30 s backoff; mtjk owns low-level TCP recovery | Exponential backoff, 10 attempts, 1–30 s | Exponential backoff, 10 attempts, 1–30 s |
| **Duplicate-send risk**             | Low–Medium                                                                                       | High                                                                                      | Medium                                   | Low                                      |
| **Outbound queue**                  | None (direct send)                                                                               | Bounded paced queue with pressure watermarks and bounded per-item retry                   | None (direct send)                       | None (router-managed)                    |
