...
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| **historical**   | _Archival documentation label._ Recorded during prior development. Not a runtime `evidence_tier` value.                                                              | "On date D, behavior X was observed." No claim about current behavior.                                                        |
| **conformance**  | Recorded against the current codebase at the current commit. Reproducible by re-running the same command.                                                            | "At commit H, behavior X is confirmed."                                                                                       |
| **synthetic**    | Recorded using `FakeAdapter`, mock objects, or simulated transport. No real network or hardware involved.                                                            | "The adapter's internal logic produces X when given input Y." No claim about real endpoint behavior.                          |
| **docker**       | Tested against a local Docker container (for example Docker Synapse). Validates SDK integration and adapter wiring but not an external network or physical hardware. | "SDK integration works in a containerized environment." Docker evidence does not prove external network or hardware behavior. |
| **live_service** | Recorded against a real external transport service over the network (for example a real Matrix homeserver or Reticulum LXMF router).                                 | "Against real endpoint E, behavior X was observed under conditions Y."                                                        |
| **hardware**     | Recorded against real physical hardware connected through serial, TCP, or BLE (for example a Meshtastic radio or MeshCore node).                                     | "Against physical device D, behavior X was observed under conditions Y."                                                      |

## 2. Classification Rules

1. Every evidence entry must include a `tier` field with one of the five runtime
   tier labels, or the archival `historical` label for prior-run documentation.
...
| ------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tier`                          | Yes      | Evidence tier: `conformance`, `synthetic`, `docker`, `live_service`, or `hardware`. `historical` is an archival documentation label for prior test runs, not a valid runtime `evidence_tier` value |
