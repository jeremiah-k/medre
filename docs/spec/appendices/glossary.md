...
| **Plugin**                | A component that observes or emits canonical events through the `Plugin` protocol. No plugin loader or runtime is implemented yet. |
...
Each domain below has one spec page that owns its semantics. The map records
the domain name, owning spec page, key source modules, and what the domain is
explicitly **not** responsible for.

| Domain             | Spec page                                    | Key modules                                                                                                                                  | Not responsible for                                                   |
| ------------------ | -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| Lifecycle          | `delivery-lifecycle.md`, `state-machines.md` | `core/engine/pipeline/delivery_state.py`, `delivery_lifecycle.py`                                                                            | Adapter delivery facts, runtime orchestration, storage DDL            |
| Adapter boundary   | `adapter-runtime.md`                         | `adapters/*/adapter.py`, `adapters/*/session.py`                                                                                             | Lifecycle state transitions, relation resolution, rendering decisions |
| Conversation graph | `event-model.md`                             | `core/planning/conversation_graph.py`, `relation_resolution.py`, `relation_enricher.py`                                                      | Adapter transport-level message IDs, rendering output format          |
| Planning           | `routing-delivery.md`                        | `core/planning/capability_decision.py`, `fallback_resolution.py`, `core/routing/router.py`                                                   | Adapter invocation, storage writes, runtime task lifecycle            |
| Operator surface   | `diagnostics-evidence.md`                    | `cli/inspect_commands.py`, `cli/trace_commands.py`, `runtime/evidence/`, `runtime/snapshot.py`                                               | Spec semantics, adapter behavior, storage mutations                   |
| Persistence        | `storage.md`                                 | `core/storage/sqlite/`                                                                                                                       | Runtime task scheduling, adapter lifecycle, evidence derivation       |
| Runtime execution  | `architecture.md` §7                         | `runtime/app.py`, `runtime/builder.py`, `runtime/retry.py`, `core/engine/pipeline/runner.py`, `core/engine/pipeline/delivery_coordinator.py` | Lifecycle vocabulary, planning decisions, storage schema              |
| Runtime evidence   | `diagnostics-evidence.md`                    | `core/evidence/`, `runtime/evidence/`, `runtime/events.py`                                                                                   | Execution authority, storage writes, adapter transport operations     |
