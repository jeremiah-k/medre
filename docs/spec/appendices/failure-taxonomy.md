...
| `dead_lettered_retryable_mismatch` | `degraded`     | outbox      | `dead_lettered` outbox item whose current lifecycle-authoritative receipt is non-terminal (`failed` or `queued`), suggesting the item may still be retryable. | The outbox says terminal but the receipt says retryable. Consider replay if the underlying cause is resolved. |
