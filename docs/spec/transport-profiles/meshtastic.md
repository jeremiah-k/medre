...
present. Resolution order is event or packet names, then local
node-metadata names, then the bare `sender_id` fallback. Event names
always win; text packets source names exclusively from `node_info`, so no
