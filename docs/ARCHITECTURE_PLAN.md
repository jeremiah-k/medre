...
- **feat/meshtastic-byte-budget-rendering** — Implemented on branch `feat/meshtastic-byte-budget-rendering`. Transport-aware rendering with a `max_text_bytes` setting (default 227, matching Meshtastic's payload budget) that UTF-8-safe byte-truncates the final rendered text to fit within the transport limit. Target-aware Meshtastic rendering is implemented: `MeshtasticRenderer` resolves config by `target_adapter`, so multiple Meshtastic adapters can have different `max_text_bytes`, `radio_relay_prefix`, and `meshnet_name` values. **Known limitation:** `MatrixRenderer` still uses the first configured Matrix adapter's config; multi-Matrix setups are deferred.
