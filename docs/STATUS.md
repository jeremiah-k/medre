...
3. **Local delivery outbox is durable but does not provide exactly-once or RF confirmation.** The outbox (`delivery_outbox` table) persists pending, retry_wait, in_progress, queued, sent, dead_lettered, cancelled, and abandoned items across process restart. However: (a) a process may crash after local adapter send succeeds but before the sent receipt is committed — recovery may resend; (b) Meshtastic adapter queue is still in-memory and non-durable — items queued but not sent at crash time are lost; (c) the outbox does not track RF confirmation, ACK, remote receipt, or end-to-end delivery. Operators can inspect aggregate outbox counts through runtime diagnostics snapshots (`medre diagnostics --refresh-health`) and can query detailed rows through SQLite/storage-level inspection. A first-class outbox inspect/recover CLI command is future work. The RetryWorker processes due outbox items when `[retry] enabled = true`.
